...
void CGameMovement::WalkMove( void )
{
	...
	if (cl_viewbob_enabled.GetInt() == 1 && !engine->IsPaused())
	{
		...
		if (pMoveData->m_bIsSprinting)
		{
			...
			zoffset = sin(gpGlobals->curtime * (cl_viewbob_timer.GetFloat()) * 1.3) * player->GetAbsVelocity().Length() * cl_viewbob_scale.GetFloat() / 100;
		}
		else
		{
			xoffset = sin(gpGlobals->curtime * cl_viewbob_timer.GetFloat()) * player->GetAbsVelocity().Length() * cl_viewbob_scale.GetFloat() * 100;
			zoffset = sin(gpGlobals->curtime * cl_viewbob_timer.GetFloat()) * player->GetAbsVelocity().Length() * cl_viewbob_scale.GetFloat() / 100;
		}

		player->ViewPunch(QAngle(xoffset * 0, 0, zoffset));
